...
func main() {
    ...
    oauthConfig = &oauth2.Config{
        ...
        RedirectURL:  "https://checkstosheets-production.up.railway.app/webhook", // Замените на ваш публичный URL
        Scopes: []string{
    ...
    // Установка Webhook
    _, err = bot.Request(tgbotapi.SetWebhookConfig{
        URL: webhookURL,
    })
    if err != nil {
    ...
    // Настройка HTTP-сервера для обработки Webhook запросов
    ...
    // Запуск HTTP-сервера
    log.Printf("Запуск HTTP сервера на %s", serverAddr)
    go func() {
        if err := http.ListenAndServeTLS(serverAddr, "cert.pem", "key.pem", nil); err != nil {
...
func startOAuthServer(errCh chan<- error) *http.Server {
    ...
    go func() {
        log.Println("Запуск OAuth2 сервера на http://checkstosheets-production.up.railway.app/")
...
func handlePhotoMessage(bot *tgbotapi.BotAPI, message *tgbotapi.Message, sheetsService *sheets.Service, spreadsheetId string, driveService *drive.Service, driveFolderId string, adminChatID int64) {
    ...
    if parseErr != nil {
        ...
        notifyAdmin(bot, adminChatID, fmt.Sprintf("Ошибка при парсинге сообщения: %v", parseErr))
        return
    }
    ...
    if err != nil {
        ...
        notifyAdmin(bot, adminChatID, fmt.Sprintf("Ошибка при получении файла: %v", err))
        return
    }
    ...
    if err != nil {
        ...
        notifyAdmin(bot, adminChatID, fmt.Sprintf("Ошибка при скачивании файла: %v", err))
        return
    }
    ...
    if err != nil {
        ...
        notifyAdmin(bot, adminChatID, fmt.Sprintf("Не удалось создать временный файл: %v", err))
        return
    }
    ...
    if err != nil {
        ...
        notifyAdmin(bot, adminChatID, fmt.Sprintf("Ошибка при сохранении файла: %v", err))
        return
    }
    ...
    if err != nil {
        ...
        notifyAdmin(bot, adminChatID, fmt.Sprintf("Ошибка при загрузке файла на Drive: %v", err))
        return
    }
    ...
    if err != nil {
        ...
        notifyAdmin(bot, adminChatID, fmt.Sprintf("Ошибка при записи в Google Sheets: %v", err))
        return
    }
...
func appendToSheet(service *sheets.Service, spreadsheetId string, data ParsedData) error {
    ...
    // Получение последней заполненной строки в листе 'Чеки' по столбцу B
    ...
    lastRow := 1 // Начальная строка (учитывая заголовок)
    if len(resp.Values) > 0 {
        lastRow = len(resp.Values) + 1
    }

    // Определение диапазона для вставки данных (B<lastRow>:G<lastRow>)
    rangeStr := fmt.Sprintf("'Чеки'!B%d:G%d", lastRow, lastRow)

    // Вставка данных в определенный диапазон
